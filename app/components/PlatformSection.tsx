...
export default function PlatformSection() {
  ...
  return (
    <section ref={ref} id="soluções" className="px-20 py-20">
      <div className="flex flex-col gap-16">
        ...
        <div className="reveal flex items-end justify-between">
          <h2 className="font-mono text-[80px] font-bold leading-[0.95] text-text-primary">
            Sua gestão
            <br />
            na palma
            <br />
          ...
          <a
            href="#cta"
            className="cta-btn border border-text-primary/20 px-5 py-2.5 cursor-pointer mb-2"
          >
        ...
        <div className="flex gap-0 stagger">
          {stats.map((stat) => (
            <div
              key={stat.label}
              className={`reveal card-hover flex-1 flex flex-col gap-4 p-8 ${stat.bg}`}
            >
              <span
              ...
              <AnimatedStat
                value={stat.value}
                suffix={stat.suffix}
                className={`font-mono text-[64px] font-extralight leading-none ${stat.valueColor}`}
              />
            </div>
          ))}
        </div>
